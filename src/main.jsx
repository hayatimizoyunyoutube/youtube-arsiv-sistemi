...
function HomePage() {
  ...
  return <Layout>
    <section className="hero-card showcase-hero">
      <div className="showcase-copy"><div className="version-pill">✅ {VERSION} • SQL reset ve temel tablolar</div><h1>Demolar kalktı. Artık gerçek Supabase tabloları var.</h1><p>Oyunlar, kullanıcı profilleri, seriler, bölümler, kategoriler, kanallar, takvim, notlar, bakım ve veri sağlığı için temiz başlangıç kuruldu.</p><div className="hero-actions"><a className="primary-btn" href="/admin">Yönetim Paneli</a><a className="ghost-btn" href="/register">Kayıt Ol</a><a className="ghost-btn" href="/status">Durumu Kontrol Et</a></div></div>
...
function AdminPage() { const session = getSession(); if (!session?.access_token) return <AuthPage mode="login" />; return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">🛡️ {VERSION} • Yönetim Paneli</div><h1>Yönetim paneli kategorileri tek sıraya alındı.</h1><p>Butonlar fotoğraftaki sıraya göre ayarlandı. Taşmaması için tek satırlı kompakt grid yapısı kullanıldı.</p></div><AdminNav /><AdminQuickManager session={session} /></section></Layout>; }
...
function Updates() { return <section className="notes-card"><h2>📌 {VERSION} Tamamlananlar</h2><ul><li>✅ Supabase tabloları sıfırlanacak şekilde schema.sql yenilendi.</li><li>✅ Oyunlar, kullanıcı profilleri, seriler, bölümler, takvim, notlar, bakım ve veri sağlığı tabloları eklendi.</li><li>✅ Üst menü fotoğraftaki kategori sırasına göre tek sıraya alındı.</li><li>✅ Yönetim paneli içindeki butonlar tek satıra sığacak kompakt yapıya alındı.</li><li>🟢 Supabase gerekli: schema.sql çalıştırılmalı.</li></ul></section>; }
function UpdatesPage() { return <Layout><PageHero icon="📝" title="Güncellemeler" text="v1.1.4 tamamlananlar ve sıradaki plan." /><Updates /><section className="next-card"><h2>➡️ Sonraki Plan: v1.1.5 Oyun Yönetimi Detay Formu</h2><p>Oyun kapak/banner, kategori, kanal, seri ve durum alanları detaylandırılacak.</p></section></Layout>; }
function StatusPage() { return <Layout><PageHero icon="✅" title="Site Durumu" text="Supabase ve route kontrol paneli." /><section className="status-grid"><article className="status-check-card"><strong>Supabase gerekli</strong><p>{supabaseConfig.isReady ? 'Env hazır görünüyor.' : 'Env eksik veya Vercel Redeploy yapılmadı.'}</p></article><article className="status-check-card"><strong>SQL Results</strong><p>schema.sql sonunda v1.1.4 başarı satırı Supabase Results kısmında görünmeli.</p></article><article className="status-check-card"><strong>Menü Düzeni</strong><p>Üst menü ve yönetim paneli butonları tek sıraya alındı.</p></article></section></Layout>; }
