...
function DataCard({ item, type = 'series' }) {
  ...
  const img = item.cover_url || item.poster_url || item.banner_url || item.logo_url || PLACEHOLDER;
  const description = item.description || item.summary || 'Açıklama henüz eklenmedi.';
  const count = item.episode_count || item.episodes || item.game_count || 0;
  return <a className="series-card upgraded-series-card compact-public-card" href={`/${type}/${slug}`}>
    <div className="series-image-wrap"><img src={img} alt={title} onError={e => { e.currentTarget.src = PLACEHOLDER; }} /><span className="status-badge">{item.status || 'Aktif'}</span></div>
    <div className="series-content"><h3>{title}</h3><p>{description}</p><div className="card-info-row"><span>{item.category_title || item.channel_title || 'Arşiv'}</span><span>{count} bölüm</span></div></div>
  </a>;
}
...
function AdminQuickManager({ session }) {
  const empty = {
    slug: '', title: '', description: '', status: 'Planlandı',
    category_slug: '', category_title: '', channel_slug: 'hayatimiz-oyun', channel_title: 'Hayatımız Oyun',
    series_slug: '', series_title: '', release_date: '', cover_url: '', banner_url: '', logo_url: '', media_note: '',
    rawg_id: '', steam_appid: '', metacritic: '', rating: '', genres: '', tags: '', playlist_url: '', playlist_id: '',
    sort_order: 100, is_public: true
  };
  const tagOptions = ['Türkçe Altyazılı', 'Türkçe Dublajlı', 'Coop', 'DLC', '%100'];
  const [rows, setRows] = useState([]);
  ...
  function slugify(value) {
    return String(value || '').trim().toLowerCase()
      ...
      .replaceAll(' ', '-').replace(/[^a-z0-9-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');
  }
  ...
  async function autoFillRawg() {
    ...
    if (!query) return setMsg('Önce oyun adını yazmalısın. Örn: 007 First Light');
    setAutoLoading(true); setMsg('RAWG + Steam bilgileri otomatik çekiliyor...');
    try {
      const response = await fetch('/api/rawg-search?query=' + encodeURIComponent(query));
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'RAWG/Steam otomatik çekme hatası.');
      const item = data.best || data.results?.[0];
      if (!item) throw new Error('Oyun bulunamadı. Oyun adını biraz daha açık yaz.');
      const gameTitle = item.name || query;
      ...
      setForm(prev => ({
        ...
        channel_slug: prev.channel_slug || 'hayatimiz-oyun',
        series_title: item.series_title || gameTitle,
        series_slug: slugify(item.series_title || gameTitle),
        cover_url: cover,
        banner_url: banner,
        logo_url: logo,
        rawg_id: String(item.id || ''),
        steam_appid: String(steamAppId || ''),
  ...
  async function syncPlaylistEpisodes(gamePayload) {
    const playlistId = gamePayload.playlist_id || extractPlaylistId(gamePayload.playlist_url);
    if (!playlistId) return { saved: 0, error: '' };
    const response = await fetch(`/api/youtube-playlist?playlistId=${encodeURIComponent(playlistId)}&playlistUrl=${encodeURIComponent(gamePayload.playlist_url || '')}`);
    const json = await response.json().catch(() => ({}));
    if (!response.ok) return { saved: 0, error: json?.error || 'YouTube API hatası. Vercel içinde YOUTUBE_API_KEY ve YouTube Data API v3 açık olmalı.' };
    if (!Array.isArray(json.items) || json.items.length === 0) return { saved: 0, error: 'Playlist bulundu ama bölüm gelmedi. Playlist herkese açık mı, URL doğru mu ve YOUTUBE_API_KEY aktif mi kontrol et.' };

    let saved = 0;
    const errors = [];
    for (const item of json.items || []) {
      const episodeSlug = `${gamePayload.slug}-${item.position || saved + 1}-${item.youtube_video_id}`.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-');
      const payload = {
        slug: episodeSlug,
        ...
        game_slug: gamePayload.slug,
        game_title: gamePayload.title,
        series_slug: gamePayload.series_slug,
        series_title: gamePayload.series_title,
        status: 'Yayında',
        published_at: item.published_at,
        is_public: true
      };
      const gameEpisode = await upsertRow('game_episodes', payload, session, 'youtube_video_id');
      const publicEpisode = await upsertRow('public_episodes', payload, session, 'youtube_video_id');
      if (!gameEpisode.error || !publicEpisode.error) saved += 1;
      if (gameEpisode.error && publicEpisode.error) errors.push(`${payload.title}: ${publicEpisode.error || gameEpisode.error}`);
    }

    const countPayload = { episode_count: saved, episodes: saved };
    await upsertRow('youtube_playlists', {
      slug: gamePayload.playlist_id || playlistId,
      title: `${gamePayload.title} Oynatma Listesi`,
      playlist_id: playlistId,
      playlist_url: gamePayload.playlist_url || `https://www.youtube.com/playlist?list=${playlistId}`,
      game_slug: gamePayload.slug,
      game_title: gamePayload.title,
      series_slug: gamePayload.series_slug,
      series_title: gamePayload.series_title,
      channel_slug: gamePayload.channel_slug,
      channel_title: gamePayload.channel_title,
      sync_status: saved ? 'Tamamlandı' : 'Boş',
      last_sync_note: errors.length ? errors.slice(0, 3).join(' | ') : `${saved} bölüm ve thumbnail çekildi.`,
      episode_count: saved,
    ...
    await upsertRow('public_series', {
      slug: gamePayload.series_slug,
      title: gamePayload.series_title,
      description: gamePayload.description || `${gamePayload.series_title} serisi.`,
      category_slug: gamePayload.category_slug,
      category_title: gamePayload.category_title,
      channel_slug: gamePayload.channel_slug,
      channel_title: gamePayload.channel_title,
      cover_url: gamePayload.cover_url,
      banner_url: gamePayload.banner_url,
      logo_url: gamePayload.logo_url,
      status: gamePayload.status,
      sort_order: Number(gamePayload.sort_order) || 100,
      is_public: true,
      game_count: 1,
      ...countPayload
    }, session, 'slug');

    await upsertRow('public_games', {
      ...gamePayload,
      playlist_id: playlistId,
      playlist_url: gamePayload.playlist_url || `https://www.youtube.com/playlist?list=${playlistId}`,
      ...countPayload
    ...
    return { saved, error: errors.length ? errors.slice(0, 2).join(' | ') : '' };
  }
  async function submit(e) {
    e.preventDefault();
    ...
    await upsertRow('public_channels', { slug: channelSlug, title: payload.channel_title, description: 'Hayatımız Oyun içerik kanalı.', channel_type: 'YouTube', is_public: true, sort_order: 100 }, session, 'slug');
    await upsertRow('public_series', { slug: seriesSlug, title: payload.series_title, description: payload.description, category_slug: categorySlug, category_title: payload.category_title, channel_slug: channelSlug, channel_title: payload.channel_title, cover_url: payload.cover_url, banner_url: payload.banner_url, logo_url: payload.logo_url, status: payload.status, sort_order: payload.sort_order, is_public: true }, session, 'slug');
    const yt = await syncPlaylistEpisodes(payload);
    setMsg(edit ? `Başarı: oyun güncellendi. Seri/kategori/kanal bağlantıları korundu. ${yt.saved ? yt.saved + ' YouTube bölümü çekildi.' : ''}${yt.error ? ' YouTube uyarısı: ' + yt.error : ''}` : `Başarı: oyun eklendi. Seri/kategori/kanal otomatik oluşturuldu. ${yt.saved ? yt.saved + ' YouTube bölümü çekildi.' : ''}${yt.error ? ' YouTube uyarısı: ' + yt.error : ''}`);
    setEdit(null); setForm(empty); load(true);
  }
  async function del(row) {
    if (!confirm(`${row.title} silinsin mi? Seri/bölüm kayıtları silinmez.`)) return;
    const r = await deleteRow('public_games', row.id, session);
    setMsg(r.error || 'Başarı: oyun silindi. Seri/bölüm kayıtları korunur.');
    load();
  }

  return <section className="admin-grid"><form className="admin-card login-card" onSubmit={submit}><h2>🎮 Oyun Ekle / Otomatik Çekme Merkezi</h2><p>Tek merkez burasıdır: RAWG + Steam kapakları + kategori/seri + YouTube bölümleri bu formdan otomatik kurulur.</p>
    <div className="form-two-col"><label>Oyun Adı<input value={form.title} onChange={e => set('title', e.target.value)} required placeholder="Örn. 007 First Light" /></label><label>Slug<input value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="boş kalırsa otomatik" /></label></div>
    <button type="button" className="ghost-btn full-btn" onClick={autoFillRawg} disabled={autoLoading}>{autoLoading ? 'Otomatik çekiliyor...' : '🎮 RAWG + Steam’den Otomatik Çek'}</button>
    <label>Hikaye / Açıklama<textarea rows="5" value={form.description} onChange={e => set('description', e.target.value)} placeholder="RAWG/Steam’den Türkçe hikaye özeti otomatik gelir." /></label>
    <div className="form-two-col"><label>Kategori Başlığı<input value={form.category_title} onChange={e => set('category_title', e.target.value)} placeholder="Aksiyon" /></label><label>Kategori Slug<input value={form.category_slug} onChange={e => set('category_slug', e.target.value)} placeholder="aksiyon" /></label></div>
    <p className="field-note">Kategori: Oyunun tür grubudur. Oyun eklenince kategori tablosuna otomatik bağlanır.</p>
    ...
    <p className="field-note">Kanal: İçeriğin yayınlandığı kanal adıdır. Kullanıcı yönetimiyle karışmaz.</p>
    <div className="form-two-col"><label>Seri Başlığı<input value={form.series_title} onChange={e => set('series_title', e.target.value)} placeholder="Örn. James Bond" /></label><label>Seri Slug<input value={form.series_slug} onChange={e => set('series_slug', e.target.value)} /></label></div>
    ...
    <div className="form-two-col"><label>Türler<input value={form.genres} onChange={e => set('genres', e.target.value)} placeholder="Aksiyon, Macera, Gizlilik" /></label><label>Ortalama Puan<input value={form.metacritic || form.rating} onChange={e => set('metacritic', e.target.value)} placeholder="RAWG/Metacritic puanı" /></label></div>
    <div className="tag-button-row">{tagOptions.map(tag => <button type="button" key={tag} className={String(form.tags).includes(tag) ? 'tag-toggle active' : 'tag-toggle'} onClick={() => toggleTag(tag)}>{tag}</button>)}</div>
    ...
    <div className="media-preview-grid">
      <article><strong>Kapak Önizleme</strong><img src={form.cover_url || PLACEHOLDER} onError={e => { e.currentTarget.src = PLACEHOLDER; }} alt="Kapak önizleme" /></article>
      <article><strong>Banner Önizleme</strong><img src={form.banner_url || PLACEHOLDER} onError={e => { e.currentTarget.src = PLACEHOLDER; }} alt="Banner önizleme" /></article>
      <article><strong>Logo Önizleme</strong><img src={form.logo_url || PLACEHOLDER} onError={e => { e.currentTarget.src = PLACEHOLDER; }} alt="Logo önizleme" /></article>
    </div>
...
function AdminSeriesPage() {
  ...
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">🎬 {VERSION} • Seri Yönetimi</div><h1>RAWG Oyun Bilgisi Hazırlığı</h1><p>Seri ekle, düzenle, sil ve oyunları seçili seriye bağla. SQL veri sıfırlamaz.</p></div><AdminNav />
...
function AdminEpisodesPage() {
  ...
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">▶️ {VERSION} • Yayın Takvimi</div><h1>RAWG Oyun Bilgisi Hazırlığı</h1><p>Oyunlara bağlı bölüm ekle, YouTube bağlantısı gir, yayın durumunu ve bölüm sırasını yönet.</p></div><AdminNav />
...
function AdminChannelsPage() {
  ...
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">📺 {VERSION} • Yayın Takvimi</div><h1>RAWG Oyun Bilgisi Hazırlığı</h1><p>Kanal ekle, düzenle, sil; logo, kapak, banner, bağlantı ve YouTube kanal ID bilgilerini yönet. SQL veri sıfırlamaz.</p></div><AdminNav />
...
function ProfilePage() {
  ...
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">👤 {VERSION} • RAWG Oyun Bilgisi Hazırlığı</div><h1>Profil ve hesap bilgileri</h1><p>Bu ekran app_users tablosundan okur. Supabase SQL gerekli, yetkiler ve veriler sıfırlanmaz.</p></div>
...
function Updates() { return <section className="notes-card"><h2>📌 {VERSION} Tamamlananlar</h2><ul><li>✅ RAWG Oyun Bilgisi Hazırlığı dolduruldu.</li><li>✅ Kullanıcı e-posta, görünen ad, rol ve durum bilgisi gösterildi.</li><li>✅ Profil görünen adı panelden kaydedilebilir hale geldi.</li><li>✅ Oturum ve yetki bilgisi Türkçe gösterildi.</li><li>✅ Supabase SQL gerekli; game_episodes YouTube alanları eklenir.</li><li>✅ Vercel/GitHub sürüm etiketi v1.3.2 olarak güncellendi.</li></ul></section>; }

function UpdatesPage() { return <Layout><PageHero icon="📝" title="Güncellemeler" text="v1.3.2 tamamlananlar ve sıradaki plan." /><Updates /><section className="next-card"><h2>➡️ Sonraki Plan: v1.3.2 RAWG Oyun Bilgisi Hazırlığı</h2><p>YouTube API anahtarı Vercel ortam değişkenlerine eklenerek playlistten bölüm çekme işlemi başlatıldı.</p></section></Layout>; }
function StatusPage() { return <Layout><PageHero icon="✅" title="Site Durumu" text="RAWG Oyun Bilgisi Hazırlığı kontrol paneli." /><section className="status-grid"><article className="status-check-card"><strong>Supabase SQL gerekli</strong><p>youtube_playlists tablosu ve playlist kolonları eklenir. Veri/yetki sıfırlanmaz.</p></article><article className="status-check-card"><strong>Yeni .env gerekli</strong><p>Vercel içine RAWG_API_KEY eklenir. Mevcut VITE_SUPABASE_URL ve VITE_SUPABASE_ANON_KEY korunur.</p></article><article className="status-check-card"><strong>YouTube API</strong><p>Playlist bölüm çekme için Vercel serverless API kullanılır.</p></article><article className="status-check-card"><strong>Veri Koruma</strong><p>DROP TABLE / TRUNCATE yoktur. Mevcut kullanıcı yetkileri, oyunlar, kategoriler, kanallar, seriler ve bölümler korunur.</p></article></section></Layout>; }
...
function AdminMaintenancePage() {
  ...
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">🛠️ {VERSION} • Bakım Modu</div><h1>RAWG Oyun Bilgisi Hazırlığı</h1><p>Siteyi bakım moduna al, kullanıcıya gösterilecek başlık/mesajı düzenle ve sosyal bağlantıları tek merkezden hazırla. SQL veri ve yetki sıfırlamaz.</p></div><AdminNav />
...
function AdminRawgPage() {
  ...
  const [loading, setLoading] = useState(false);
  useEffect(() => { if (session?.access_token) getCurrentAppUser(session).then(r => { setProfile(r.data); setLoadingProfile(false); }); else setLoadingProfile(false); }, [session?.access_token]);
  async function searchRawg(e) {
    e.preventDefault();
    if (!query.trim()) return setMsg('Oyun adı yazmalısın.');
    setLoading(true); setMsg('RAWG üzerinden aranıyor...'); setItems([]);
    try {
      const res = await fetch('/api/rawg-search?query=' + encodeURIComponent(query.trim()));
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'RAWG arama hatası.');
      setItems(data.results || []);
      setMsg(`Başarı: ${data.results?.length || 0} sonuç bulundu. Kapak/meta bilgilerini oyun yönetiminde kullanabilirsin.`);
    } catch (error) {
  ...
  if (!session?.access_token) return <AuthPage mode="login" />;
  if (loadingProfile) return <Layout><section className="admin-shell"><p>Yetki kontrol ediliyor...</p></section></Layout>;
  if (!isAdminRole(profile?.role)) return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">👤 {VERSION}</div><h1>Bu alan sadece yetkililere açıktır.</h1><p>RAWG oyun bilgisi merkezi normal kullanıcıya gösterilmez.</p></div></section></Layout>;
  return <Layout><section className="admin-shell"><div className="admin-hero"><div className="version-pill">🧩 {VERSION} • RAWG Oyun Bilgisi Hazırlığı</div><h1>RAWG oyun arama ve kapak/meta önizleme merkezi.</h1><p>Oyun eklerken doğru kapak, çıkış tarihi ve tür bilgisini bulmak için RAWG arama altyapısı hazırlandı.</p></div><AdminNav />
    <section className="status-grid"><article className="status-check-card"><strong>Yeni .env</strong><p>Gerekli: Vercel Environment Variables içine RAWG_API_KEY eklenir.</p></article><article className="status-check-card"><strong>Supabase SQL</strong><p>Gerekmez. Mevcut public_games alanları kullanılacak; tablo/kolon eklenmez.</p></article><article className="status-check-card"><strong>Veri Koruma</strong><p>Arama sadece önizleme yapar; mevcut oyunlar, kullanıcılar ve yetkiler sıfırlanmaz.</p></article></section>
    <section className="admin-grid"><form className="admin-card login-card" onSubmit={searchRawg}><h2>🎮 RAWG Oyun Ara</h2><label>Oyun Adı<input value={query} onChange={e => setQuery(e.target.value)} placeholder="Örn. Resident Evil 5" /></label><button className="primary-btn" disabled={loading}>{loading ? 'Aranıyor...' : 'RAWG Üzerinden Ara'}</button>{msg ? <p className={msg.startsWith('Başarı') ? 'form-message success' : 'form-message error'}>{msg}</p> : null}</form>
      <div className="admin-card admin-table-card"><h2>Sonuçlar</h2><table className="admin-table"><thead><tr><th>Oyun</th><th>Çıkış</th><th>Kapak</th><th>RAWG</th></tr></thead><tbody>{items.map(item => <tr key={item.id}><td><strong>{item.name}</strong><small>{item.genres?.map(g => g.name).join(', ') || 'Tür yok'}</small></td><td>{item.released || '—'}</td><td>{item.background_image ? <a href={item.background_image} target="_blank">Kapak URL</a> : '—'}</td><td>{item.id}</td></tr>)}{!items.length ? <tr><td colSpan="4">Henüz sonuç yok.</td></tr> : null}</tbody></table></div></section>
  </section></Layout>;
}
...
function AppRouter() {
  ...
  if (p === '/admin/rawg') return <AdminRawgPage />;
