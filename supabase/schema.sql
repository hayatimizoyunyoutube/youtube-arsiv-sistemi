...
-- =========================================================
-- FIX: Email not confirmed düzeltmesi
-- Bu bölüm geliştirme aşaması içindir.
-- Mevcut kullanıcıları onaylı yapar ve yeni kayıtları otomatik onaylar.
-- =========================================================
...
insert into public.site_status_logs (version, status, detail)
values ('v1.1.5-fix', 'success', 'Email not confirmed fix çalıştı: mevcut Auth kullanıcıları onaylandı ve yeni kayıtlar için auto confirm trigger eklendi.');

select
  'Email confirm fix çalıştı' as status,
  'Bu bir sürüm değildir; v1.1.5 üzerinde auth giriş/kayıt fixidir.' as not,
  'Mevcut auth.users kayıtları onaylandı, app_users senkronlandı.' as islem,
  now() as calisma_zamani;
